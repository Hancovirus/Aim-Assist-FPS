...
def get_iou(p, a):
    ...
    p_area = bbox_attr(p, 4) * bbox_attr(p, 3)                  # (batch, S, S, B)
    p_area = p_area.unsqueeze(4).expand_as(intersection)        # (batch, S, S, B, 1) -> (batch, S, S, B, B)

    a_area = bbox_attr(a, 4) * bbox_attr(a, 3)                  # (batch, S, S, B)
...
def bbox_to_coords(t):
    ...
    width = bbox_attr(t, 3)
    x = bbox_attr(t, 1)
    i = torch.arange(S).view(1, S, 1, 1)  # Shape: (1, S, 1, 1)
    x0 = i.repeat(t.size(0), 1, S, x.size(3)).to("cuda")     # Shape: (batch_size, S, S, B)
    x1 = (x+x0)/S - width / 2.0
    x2 = (x+x0)/S + width / 2.0

    height = bbox_attr(t, 4)
    y = bbox_attr(t, 2)
    i = torch.arange(S).view(1, 1, S, 1)  # Shape: (1, 1, S, 1)
    y0 = i.repeat(t.size(0), S, 1, y.size(3)).to("cuda")
    y1 = (y+y0)/S - height / 2.0
    y2 = (y+y0)/S + height / 2.0
...
def plot_boxes(data, labels, classes, color='orange', min_confidence=0.2, max_overlap=0.7, file=None):
    ...
    for i in range(m):
        for j in range(n):
            for k in range((labels.size(dim=2) - config.C) // 5):
                ...
                confidence = labels[i, j, class_index].item() * bbox[0].item()          # pr(c) * IOU
                if confidence > min_confidence:
                    width = bbox[3] * config.IMAGE_SIZE[0]
                    height = bbox[4] * config.IMAGE_SIZE[1]
                    tl = (
                        (bbox[1] + j) * grid_size_x - width / 2,
                        (bbox[2] + i) * grid_size_y - height / 2
                    )
    ...
    for i in range(num_boxes):
        if i not in discarded:
            ...
            text = f'{classes[class_index]} {round(min(confidence,1) * 100, 1)}%'
